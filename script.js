document.addEventListener('DOMContentLoaded', () => {
    ...
    // Scroll: sticky header after hero
        ...
        const type = () => {
            if (i < text.length) {
                ...
                const blink = setInterval(() => {
                    tagline.style.borderRight = blinks % 2 === 0 ? 'none' : '2px solid rgba(255,255,255,0.7)';
                    if (++blinks >= 6) { clearInterval(blink); tagline.style.borderRight = 'none'; }
                }, 400);
            }
        };
        setTimeout(type, 500);
